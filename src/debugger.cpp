...
void Debugger::printDisasmLine( u32 addr ) {
    char disStr[50];
    char disHex[9];
    u8 opLen = Disassembler::disassemble( ( mmu->memRead8( addr )<<16 )|
...
void Debugger::cmdPrintRegisters() {
    ...
    printf( " SP:%04x\n",cpu->SP.w );
    printf( " PC:%04x\n",cpu->PC.w );
    printf( " IME:%x IE:%x IF:%x\n",cpu->IME,mmu->mem[REG_IE],mmu->mem[REG_IF] );
    printf( " LY:%x\n",mmu->mem[REG_LY] );
    printf( "-----------------------------\n" );
}
